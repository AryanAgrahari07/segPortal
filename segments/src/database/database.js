...
class DatabricksSQLManager {
  ...
  async executeQuery(query, params = [], retries = MAX_RETRIES) {
    try {
      ...
      if (process.env.DATABRICKS_CATALOG && process.env.DATABRICKS_SCHEMA) {
        await this._setContext(process.env.DATABRICKS_CATALOG, process.env.DATABRICKS_SCHEMA);
      }
      ...
      if (retries > 0) {
        console.log(`🔄 Retrying query (${MAX_RETRIES - retries + 1}/${MAX_RETRIES})...`);
  ...
  async _setContext(catalog, schema) {
    try {
      ...
      console.log(`Setting context to catalog: ${sanitizedCatalog}, schema: ${sanitizedSchema}`);
      
      await this.session.executeStatement(`USE CATALOG \`${sanitizedCatalog}\``);
